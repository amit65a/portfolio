...
class TypeWriter {
    ...
    tick() {
        ...
        let delta = 100 - Math.random() * 50; // Randomize typing speed (50-100ms)

        if (!this.isDeleting && this.txt === fullTxt) {
            delta = this.period; // Pause at end of sentence
            this.isDeleting = true; // Trigger 'delete' phase (which is now instant)
        } else if (this.isDeleting) {
            // Instant delete
            this.txt = '';
            this.isDeleting = false;
            this.loopNum++;
            delta = 50; // Pause before start of next word
        }
...
function initTypeWriter() {
    if (typedTextElement) {
        new TypeWriter(typedTextElement, sentences, 50);
    }
}
